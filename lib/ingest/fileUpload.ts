...
import { videoUploaded } from "@/lib/inngest/events";
...
export async function ingestFileUpload(
  ...
  if (!input.file.type.startsWith(PREFIX)) {
    return {
      ok: false,
      error: { code: "VALIDATION", message: "Must be a video file" },
    };
  }
  if (input.file.size > MAX) {
    return {
      ...
      error: {
        code: "VALIDATION",
        message: `File too large (max ${(MAX / 1024 / 1024).toFixed(0)} MB)`,
      },
    };
  }
